...
const { getNetwork, getTestContract, getUrl } = requireBox(
  "seed-eos/tools/eos/utils"
);
  ...
  it("Oracle - feed every 2 seconds + median", (done) => {
    (async () => {
      try {
        ...
        let expectedMedian = Number.parseFloat(
          sortedFeeds[Math.floor(sortedFeeds.length / 2)].price
        );
        // even => average
        if (sortedFeeds.length % 2 === 0) {
          expectedMedian += Number.parseFloat(
            sortedFeeds[Math.floor(sortedFeeds.length / 2) - 1].price
          );
          expectedMedian /= 2;
        }
